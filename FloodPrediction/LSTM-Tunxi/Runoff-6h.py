...
from TGLSTM.ind_rnn import IndRNN
...
reframed = series_to_supervised(values, 12, 7)
...
values = scaled
n_train_hours = 30000#365*24*2=2years,548*24=1.5years

train = values[:n_train_hours, :]
test = values[n_train_hours:, :]
actual=data[n_train_hours:, :]
actual_y=actual[:, -1]
...
inv_yhat1 = concatenate((yhat1, test_X1[:, :]), axis=1)
...
inv_y1 = scaler.inverse_transform(test)
inv_y1 = inv_y1[:,-1]

# # invert scaling for actual IndRNN
# inv_y2 = scaler.inverse_transform(test_y2,test_X2)
# inv_y2 = inv_y2[:,0]
#
# # invert scaling for actual Zoneout
# inv_y3 = scaler.inverse_transform(test_y3,test_X3)
# inv_y3 = inv_y3[:,0]
...
pyplot.plot(inv_y[0:200],label='true')
pyplot.plot(inv_yhat[0:200],'r--',label='predictions_LSTM')
# pyplot.plot(inv_yhat2[:1000],'c-.',label='predictions_IndRNN')
# pyplot.plot(inv_yhat3[:1000],'k:',label='predictions_LSTM+Zoneout')
pyplot.plot(inv_yhat1[0:200],'g--',label='predictions_TG-LSTM')
pyplot.legend()
pyplot.savefig('Tunxi 436D', dpi=300)
pyplot.show()